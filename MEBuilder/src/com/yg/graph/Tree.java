...
public class Tree<T> {
    ...
    public ArrayList<Node<T>> getLongestPathFromRootToAnyLeaf() {
        ...
        int maxSize = 0, maxWeight = 0, pathWeight = 0;
        for (ArrayList<Node<T>> path : getPathsFromRootToAnyLeaf()) {
        	pathWeight = calculatePathWeight(path);
        	System.out.println("PATH weight: " + pathWeight);
            if (path.size() > maxSize && pathWeight > maxWeight) {
            	maxSize = path.size();
    ...
    public int calculatePathWeight(ArrayList<Node<T>> path) {
    	int totWeight = 0;
